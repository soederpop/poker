...
3. Join the table with your agent:
   \`luca poker join ws://127.0.0.1:3001 --token <token> --agent ./${options.slug}\`
...
function newAgentStrategyContent(options: { actorTemplate: string | null }): string {
  ...
  return `// ──────────────────────────────────────────────────────────────────
// Available globals (injected by pokurr when your bot joins a game):
//
//   equity(hands, board?, iterations?)   — hand vs hand equity
//   equityEngine                         — full equity engine (JS + WASM backends)
//   evaluateHand(cards)                  — evaluate best 5-card hand from 5-7 cards
//   compareHands(a, b)                   — compare two HandRank results
//   HandEquity                           — new HandEquity("AhKd") for combo analysis
//   Range                                — new Range("QQ+,AKs") for range math
//   RangeEquity                          — range vs range equity calculation
//   compareRanges(ours, theirs, opts)    — quick range vs range comparison
//   Deck                                 — new Deck(), .shuffle(), .draw()
//   stringToCard(s) / cardToString(c)    — card parsing
//   normalizeCombo(cards)                — "AcKd" → "AKo"
//   strategy                             — built-in strategy engine (profiles, estimateEquity)
// ──────────────────────────────────────────────────────────────────
...
// Declare globals so TypeScript doesn't complain
declare const equity: any
declare const Range: any
declare const evaluateHand: any
declare const normalizeCombo: any
...
async function runNewAgent(container: AGIContainer & any, options: PokerOptions, args: string[]) {
  ...
  for (const f of ["README.md", "strategy.ts", "docs/situations/README.md"]) {
...
async function runAdminCreateTable(container: AGIContainer & any, options: PokerOptions, args: string[]) {
  ...
  let serverBaseUrl = String(options.server || "").trim()
  ...
  if (!serverBaseUrl) {
    serverBaseUrl = `http://127.0.0.1:${options.port}`
  }

  if (!name) {
    throw new Error('Table name is required. Example: pokurr admin create-table "SNG 100" --blinds 1/2')
  }
  ...
  const response = await rest.post("/api/v1/tables", payload) as Record<string, any>
...
async function runAdminListTables(container: AGIContainer & any, options: PokerOptions, args: string[]) {
  ...
  const rest = container.client("rest", { baseURL: serverBaseUrl, json: true })
  const response = await rest.get("/api/v1/tables") as Record<string, any>
