...
export const description = "Poker command suite (analyze, sim, serve, seed, join, watch, new-agent, types)"
...
function printUsage(container: AGIContainer & any, standalone = false) {
  ...
  group("♦ Admin")
  usage('admin create-table "SNG 100" [1/2]', "Create a table on the server")
  usage("admin tables", "List all tables")
...
function newAgentReadmeContent(options: { slug: string; displayName: string; actorTemplate: string | null; standalone?: boolean }): string {
  ...
  return JSON.stringify(
    {
      ...
      include: ["strategy.ts", "types/**/*.d.ts"],
    },
    null,
    2,
  ) + "\n"
...
async function runTypes(container: AGIContainer & any, _options: PokerOptions, args: string[]) {
  const standalone = isStandaloneMode(container)
  const fs = container.feature("fs", { enable: true })
  const ui = container.feature("ui")

  // Determine target directory: explicit arg, or CWD
  const target = args[0]
    ? (standalone ? resolve(process.cwd(), args[0]) : container.paths.resolve(args[0]))
    : process.cwd()

  // Validate it looks like an agent project (has strategy.ts)
  const strategyPath = join(target, "strategy.ts")
  if (!(await fs.existsAsync(strategyPath))) {
    throw new Error(`No strategy.ts found in ${target} — run this from an agent directory or pass the path as an argument.`)
  }

  fs.ensureFolder(join(target, "types"))
  await fs.writeFileAsync(join(target, "types", "pokurr.d.ts"), newAgentTypesContent())

  // Write tsconfig if it doesn't exist yet
  const tsconfigPath = join(target, "tsconfig.json")
  if (!(await fs.existsAsync(tsconfigPath))) {
    await fs.writeFileAsync(tsconfigPath, newAgentTsconfigContent())
    printSection(container, "Types Updated")
    printKV(ui, "Directory", ui.colors.cyan(target))
    console.log(ui.colors.dim("  ♦ types/pokurr.d.ts (written)"))
    console.log(ui.colors.dim("  ♦ tsconfig.json (created)"))
  } else {
    printSection(container, "Types Updated")
    printKV(ui, "Directory", ui.colors.cyan(target))
    console.log(ui.colors.dim("  ♦ types/pokurr.d.ts (written)"))
    console.log(ui.colors.dim("  ♦ tsconfig.json (already exists, skipped)"))
  }
}
...
async function runJoin(container: AGIContainer & any, options: PokerOptions, args: string[]) {
  ...
  // Load agent strategy: either custom strategy.ts from --agent path, or built-in profile
  let agentDecide: ((context: any) => any) | null = null
  let strategyLabel = ""

  if (options.agent) {
    const agentPath = resolve(process.cwd(), options.agent)
    const strategyFile = join(agentPath, "strategy.ts")
    const fs = container.feature("fs", { enable: true })
    if (!(await fs.existsAsync(strategyFile))) {
      throw new Error(`Agent strategy not found: ${strategyFile}`)
    }

    // Load strategy.ts in a VM with poker tools injected as globals
    const vmFeature = container.feature("vm", { enable: true }) as any
    const pokurrCore = await import("@pokurr/core")
    const strategyFeatureForAgent = container.feature("strategy", { enable: true }) as any

    const mod = vmFeature.loadModule(strategyFile, {
      ...
      // Full container access for advanced strategies
      container,
    ...
    if (typeof mod.decide !== "function") {
      throw new Error(`Agent strategy.ts must export a decide(context) function`)
    }
    agentDecide = mod.decide
    strategyLabel = `agent:${options.agent}`
  }

  const strategyFeature = !agentDecide ? container.feature("strategy", { enable: true }) as any : null
  ...
  const preferredProfile = strategyMap.hero
    ...
    || "tight-aggressive"
  if (!agentDecide) {
    strategyLabel = preferredProfile
  }
    ...
    if (message.type === "action_on_you") {
      ...
      ;(async () => {
        try {
          ...
          let strategyDecision: { action: string; amount?: number; reason?: string }

          if (agentDecide) {
            // Call user's custom decide() with the scaffold's DecisionContext shape
            strategyDecision = await agentDecide({
              heroCards,
            ...
            strategyDecision = await strategyFeature.decide(preferredProfile, {
              heroCards,
              board: liveState.board,
              street,
              position: liveState.position,
  ...
  printKV(ui, "Strategy", ui.colors.cyan(strategyLabel))
...
type DashboardView = "lobby" | "spectate"
type TableFilter = "all" | "cash" | "tournament" | "active"
...
interface DashboardState {
  ...
  live: {
    ...
    pot: number
    board: string[]
    players: Array<{ name: string; seat: number; stack: number; inHand?: boolean; folded?: boolean; allIn?: boolean; committed?: number; cards?: string[] }>
...
const FILTERS: TableFilter[] = ["all", "cash", "tournament", "active"]

function createDashboardState(serverUrl: string): DashboardState {
  return {
    view: "lobby",
    filter: "all",
    cursor: 0,
...
function filteredTables(state: DashboardState): Array<any> {
  const tables = state.tables || []
  if (state.filter === "all") return tables
  if (state.filter === "active") return tables.filter((t: any) => t.handActive || (Array.isArray(t.players) && t.players.length >= 2))
  if (state.filter === "cash") return tables.filter((t: any) => !String(t.name || "").toLowerCase().includes("sng") && !String(t.name || "").toLowerCase().includes("tournament"))
  if (state.filter === "tournament") return tables.filter((t: any) => String(t.name || "").toLowerCase().includes("sng") || String(t.name || "").toLowerCase().includes("tournament"))
  return tables
}

function renderLobby(state: DashboardState, c: any, cols: number): string {
  const lines: string[] = []
  const w = Math.max(cols, 60)
  const tables = filteredTables(state)

  // Header
  lines.push("")
  lines.push(c.bold.red("♠") + c.bold.white(" POKURR DASHBOARD ") + c.bold.red("♠") + c.dim(`  ${state.serverUrl}`))
  lines.push("")

  // Filter tabs
  const tabs = FILTERS.map((f, i) => {
    const label = ` ${f.toUpperCase()} `
    if (f === state.filter) return c.bgWhite.black.bold(label)
    return c.dim(label)
  }).join(c.dim(" │ "))
  lines.push(`  ${tabs}` + c.dim("    ← / → to switch filters"))
  lines.push("")

  // Table header
  const hdr = `  ${c.dim("#".padEnd(3))} ${c.bold("Table".padEnd(28))} ${c.bold("Blinds".padEnd(10))} ${c.bold("Players".padEnd(10))} ${c.bold("Status".padEnd(10))} ${c.bold("Hand")}`
  lines.push(hdr)
  lines.push(c.dim("  " + "─".repeat(Math.min(w - 4, 80))))

  if (tables.length === 0) {
    lines.push(c.dim("  No tables match this filter."))
  }

  for (let i = 0; i < tables.length; i++) {
    const t = tables[i]
    const selected = i === state.cursor
    const prefix = selected ? c.cyan.bold(" ▸ ") : "   "
    const name = String(t.name || t.id || "???").slice(0, 26).padEnd(28)
    const blinds = `${t.smallBlind || "?"}/${t.bigBlind || "?"}`.padEnd(10)
    const playerCount = Array.isArray(t.players) ? t.players.length : 0
    const players = `${playerCount}/${t.maxPlayers || "?"}`.padEnd(10)
    const status = String(t.status || "unknown").padEnd(10)
    const hand = t.handActive ? c.green("●") : c.dim("○")

    const nameStyled = selected ? c.cyan.bold(name) : c.white(name)
    const statusStyled = status.trim() === "active" ? c.green(status) : status.trim() === "closed" ? c.red(status) : c.yellow(status)

    lines.push(`${prefix}${c.dim(String(i + 1).padEnd(3))} ${nameStyled} ${blinds} ${players} ${statusStyled} ${hand}`)
  }

  // Player detail for selected table
  if (tables.length > 0 && state.cursor < tables.length) {
    const selected = tables[state.cursor]
    const seatPlayers = Array.isArray(selected.players) ? selected.players : []
    if (seatPlayers.length > 0) {
      lines.push("")
      lines.push(c.dim("  " + "─".repeat(Math.min(w - 4, 80))))
      lines.push(c.bold.cyan(`  ♦ Seated at: ${selected.name || selected.id}`))
      for (const p of seatPlayers) {
        const nameStr = String(p.name || p.botId || "???").padEnd(20)
  ...
  lines.push("")
  lines.push(c.dim("  ↑/↓ navigate  ENTER spectate  TAB/←/→ filter  q quit"))
...
function renderSpectate(state: DashboardState, c: any, cols: number): string {
  const lines: string[] = []
  ...
  const stageColors: Record<string, any> = {
    preflop: c.white, flop: c.cyan, turn: c.yellow, river: c.redBright, showdown: c.green, waiting: c.dim,
  }
  const stagePaint = stageColors[live.stage] || c.white

  lines.push("")
  lines.push(c.bold.red("♠") + c.bold.white(" POKURR ") + c.dim("│") + c.bold(` Table ${live.tableId} `) + c.dim("│") + ` Hand ${c.bold(`#${live.handNumber}`)} ` + c.dim("│") + ` ${stagePaint(String(live.stage).toUpperCase())} ` + c.dim("│") + c.green.bold(` Pot ${live.pot}`))
  lines.push("")

  // Board
  const boardText = live.board.length > 0 ? prettyCards(live.board, c) : c.dim("  --- awaiting flop ---")
  lines.push(`  Board: ${boardText}`)
  lines.push("")

  // Table visualization
  lines.push(c.dim("  " + "─".repeat(Math.min(w - 4, 70))))
  const players = [...live.players].sort((a, b) => a.seat - b.seat)

  if (players.length === 0) {
    lines.push(c.dim("  Waiting for players..."))
  }

  for (const p of players) {
    const isDealer = p.seat === live.dealerSeat
    const isActor = p.name === live.currentActor
    const dealerBadge = isDealer ? c.yellow(" Ⓓ") : "  "
    const actorBadge = isActor ? c.cyan.bold(" ◀") : "  "

    const seatStr = c.dim(String(p.seat).padStart(2) + ".")
    const nameRaw = String(p.name || "???")
    const nameStr = isActor ? c.cyan.bold(nameRaw.padEnd(18)) : nameRaw.padEnd(18)
    const stackStr = c.green(String(p.stack).padStart(8))
    const committed = p.committed ? c.yellow(` (${p.committed})`) : ""

    const cards = Array.isArray(p.cards) && p.cards.length === 2
      ? " " + prettyCards(p.cards, c)
      : p.inHand && !p.folded
        ? " " + c.dim("[??]")
        : ""

    const flags = [
      p.inHand === false ? c.red("OUT") : "",
      p.folded ? c.dim("FOLD") : "",
      p.allIn ? c.yellow.bold("ALL-IN") : "",
    ].filter(Boolean).join(" ")

    lines.push(`  ${seatStr}${dealerBadge} ${nameStr} ${stackStr}${committed}  ${flags}${cards}${actorBadge}`)
  }
  ...
  // Feed
  lines.push("")
  lines.push(c.bold.yellow("  Action Feed"))
  const feedSlice = state.feed.slice(0, 12)
  if (feedSlice.length === 0) {
    lines.push(c.dim("    Waiting for action..."))
  }
  for (const entry of feedSlice) {
    lines.push(`    ${entry}`)
  }

  lines.push("")
  lines.push(c.dim("  ESC back to lobby  q quit"))
...
async function runDashboard(container: AGIContainer & any, options: PokerOptions, args: string[]) {
  const explicit = String(args[0] || options.server || "").trim()
  const serverBaseUrl = explicit || `http://127.0.0.1:${options.port}`
  const ui = container.feature("ui")
  const c = ui.colors

  const state = createDashboardState(serverBaseUrl)

  // Resolve spectator WS URL from health endpoint
  const rest = container.client("rest", { baseURL: serverBaseUrl, json: true })
  let health: any
  try {
    health = await rest.get("/api/v1/health")
    state.spectatorWsUrl = String(health.spectatorWsUrl || "").trim()
  } catch (err: any) {
    throw new Error(`Cannot reach server at ${serverBaseUrl}: ${err.message || err}`)
  }

  if (!state.spectatorWsUrl) {
    throw new Error("Server does not expose a spectator websocket. Start with --spectatorPort.")
  }

  // Fetch tables
  async function refreshTables() {
    try {
      const payload = await rest.get("/api/v1/tables") as any
      state.tables = Array.isArray(payload.tables) ? payload.tables : []
      state.error = null
    } catch (err: any) {
      state.error = `Failed to fetch tables: ${err.message || err}`
    }
  }
  ...
  function render() {
    ...
    const frame = state.view === "lobby"
      ? renderLobby(state, c, cols)
      : renderSpectate(state, c, cols)
  ...
  // Spectator client
  let spectatorClient: InstanceType<typeof PokerClient> | null = null

  async function startSpectating(tableId: string) {
    state.view = "spectate"
    state.spectating = tableId
    state.live = { tableId, handNumber: 0, stage: "waiting", pot: 0, board: [], players: [], currentActor: null, dealerSeat: 0 }
    state.feed = []
    render()

    spectatorClient = new PokerClient(container, { wsUrl: state.spectatorWsUrl, reconnect: true })

    spectatorClient.onMessage((msg) => {
      if (msg.type === "spectator_state") {
        const p = msg.payload || {} as any
        state.live.tableId = String(p.tableId || state.live.tableId)
        state.live.handNumber = Number(p.handNumber || state.live.handNumber || 0)
        state.live.stage = String(p.stage || state.live.stage || "waiting")
        state.live.pot = Number(p.pot || 0)
        state.live.board = Array.isArray(p.board) ? p.board.map((x: unknown) => String(x)) : state.live.board
        state.live.currentActor = p.currentActor ? String(p.currentActor) : null
        state.live.dealerSeat = Number(p.dealerSeat || 0)
        state.live.players = Array.isArray(p.players)
          ? p.players.map((e: any) => ({
            name: String(e?.name || e?.botId || "player"),
            seat: Number(e?.seat || 0),
            stack: Number(e?.stack || 0),
            inHand: Boolean(e?.inHand),
            folded: Boolean(e?.folded),
            allIn: Boolean(e?.allIn),
            committed: Number(e?.committed || 0),
            cards: Array.isArray(e?.holeCards) ? e.holeCards.map(String) : undefined,
          }))
          : state.live.players
        render()
      }
      ...
      if (msg.type === "hand_result") {
        const winners = Array.isArray(msg.payload?.winners) ? (msg.payload!.winners as any[]) : []
        const summary = winners.map((w: any) => `${w.playerId || ""} +${w.amount || 0}`).join(", ")
        state.feed.unshift(c.green.bold(`🏆 pot=${msg.payload?.pot || 0} → ${summary || "none"}`))

        // Reveal showdown cards
        if (Array.isArray(msg.payload?.showdown)) {
          for (const sd of msg.payload!.showdown as any[]) {
            const player = state.live.players.find((p) => p.name === String(sd.playerId || ""))
            if (player && Array.isArray(sd.cards)) {
              player.cards = sd.cards.map(String)
            }
          }
        }
      ...
      if (msg.type === "error") {
        state.feed.unshift(c.red(`✗ ${msg.payload?.message || "unknown error"}`))
        render()
      }
    })
    ...
    try {
      ...
      await spectatorClient.send("spectate", { tableId })
      await spectatorClient.waitFor("spectator_joined", 10_000)
    } catch (err: any) {
      state.error = `Spectator connection failed: ${err.message || err}`
      render()
    }
  }
  ...
  async function stopSpectating() {
    if (spectatorClient) {
      try { await spectatorClient.disconnect() } catch { /* ignore */ }
      spectatorClient = null
    }
    state.view = "lobby"
    state.spectating = null
    await refreshTables()
    state.cursor = Math.min(state.cursor, Math.max(0, filteredTables(state).length - 1))
    render()
  }

  // Auto-refresh tables every 5s in lobby
  ...
  await new Promise<void>((resolve) => {
    ...
    stdin.on("data", async (key: string) => {
      ...
      if (state.view === "lobby") {
        ...
        // Arrow down / j
        if (key === "\x1b[B" || key === "j") {
          state.cursor = Math.min(tables.length - 1, state.cursor + 1)
          render()
          return
        }
        // Arrow left / shift+tab
        ...
        if (key === "\r" || key === "\n") {
          if (tables.length > 0 && state.cursor < tables.length) {
            const tableId = String(tables[state.cursor].id)
            await startSpectating(tableId)
          }
          return
        }
        // r to refresh
        if (key === "r") {
          await refreshTables()
          state.cursor = Math.min(state.cursor, Math.max(0, tables.length - 1))
          render()
          return
        }
      }

      if (state.view === "spectate") {
        // Escape or backspace = back to lobby
...
async function runAdmin(container: AGIContainer & any, options: PokerOptions, args: string[]) {
  const adminSubcommand = String(args[0] || "").toLowerCase()

  if (adminSubcommand === "create-table") {
    await runAdminCreateTable(container, options, args.slice(1))
    return
  }

  if (adminSubcommand === "tables" || adminSubcommand === "list-tables") {
    await runAdminListTables(container, options, args.slice(1))
    return
  }
  ...
  console.log(`    ${c.cyan(cmd + " admin create-table")}  ${c.dim('Create a table (e.g. --name "SNG 100" --blinds 1/2)')}`)
  console.log(`    ${c.cyan(cmd + " admin tables")}         ${c.dim("List all tables on the server")}`)
  console.log("")
}

function resolveAdminSocketPath(container: AGIContainer & any, options: PokerOptions): string {
  return container.paths.resolve("tmp", `pokurr-admin-${options.port}.sock`)
}
...
async function adminRequest(container: AGIContainer & any, socketPath: string, command: Record<string, any>): Promise<Record<string, any>> {
  ...
  const response = await new Promise<Record<string, any>>((resolve, reject) => {
    const timeout = setTimeout(() => {
      reject(new Error("Admin request timed out (5s)"))
    }, 5000)
...
async function runAdminCreateTable(container: AGIContainer & any, options: PokerOptions, args: string[]) {
  const ui = container.feature("ui")
  const c = ui.colors
  ...
  if (!name) {
    throw new Error('Table name is required. Example: pokurr admin create-table "SNG 100" 1/2')
  }

  const socketPath = resolveAdminSocketPath(container, options)
  const payload: Record<string, any> = { name }

  if (blindsStr) {
    payload.blinds = blindsStr
  }

  const response = await adminRequest(container, socketPath, {
    action: "create_table",
    payload,
  })

  if (!response?.ok) {
    throw new Error(`Failed to create table: ${response?.error || "unknown error"}`)
  }

  const table = response.table as Record<string, any>

  printSection(container, "Table Created")
  printKV(ui, "Table ID", c.bold(String(table.id)))
  printKV(ui, "Name", c.white(String(table.name)))
  printKV(ui, "Blinds", c.dim(`${table.smallBlind}/${table.bigBlind}`))
  printKV(ui, "Starting Stack", c.dim(String(table.startingStack)))
  printKV(ui, "Max Players", c.dim(String(table.maxPlayers)))
  printKV(ui, "Action Timeout", c.dim(`${table.actionTimeout}s`))
  console.log("")
}

async function runAdminListTables(container: AGIContainer & any, options: PokerOptions, args: string[]) {
  const ui = container.feature("ui")
  const c = ui.colors

  const socketPath = resolveAdminSocketPath(container, options)
  const response = await adminRequest(container, socketPath, { action: "list_tables" })

  if (!response?.ok) {
    throw new Error(`Failed to list tables: ${response?.error || "unknown error"}`)
  }

  const tables = Array.isArray(response?.tables) ? response.tables : []

  printSection(container, "Tables")
  printKV(ui, "Count", c.bold(String(tables.length)))
  ...
  if (tables.length === 0) {
    ...
    for (const table of tables) {
      ...
      const statusColor = status === "running" ? c.green : status === "registration" ? c.yellow : c.dim
      console.log(
        `  ${c.white(String(table.name || table.id).padEnd(20))} ` +
        `${c.dim(String(table.blinds).padEnd(8))} ` +
        `${c.dim(`${players.length}/${table.maxPlayers}`).padEnd(12)} ` +
        statusColor(status)
      )
    }
  }
...
export async function handler(options: PokerOptions, context: ContainerContext) {
  ...
  if (subcommand === "types") {
    await runTypes(container, options, args.slice(2))
    return
  }
  ...
  if (subcommand === "dashboard") {
    await runDashboard(container, options, args.slice(2))
    return
  }
  ...
  if (subcommand === "admin") {
    await runAdmin(container, options, args.slice(2))
    return
  }
